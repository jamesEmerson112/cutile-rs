...
This checkout is a personal fork of NVIDIA's research project, not the upstream repo. `origin` is `jamesEmerson112/cutile-rs` and `upstream` is `NVlabs/cutile-rs`. The fork's `main` is upstream's `main` plus a few fork-only files: this `CLAUDE.md`, the `pod/` directory, and `notes/` (dated HTML learning notes, one topic per file, published as Claude artifacts; raw material a note quotes lives in a sibling directory with the same date prefix, with a README on how it was produced).

The user is learning Rust and this project at the same time, and already knows the basics of CUDA: kernels, thread blocks and grids, `blockIdx` and `threadIdx`, streams, `nvcc`. Explain cuTile ideas by mapping them onto those CUDA terms first (a tile program is the cousin of a thread block, `program_id` of `blockIdx`, Tile IR of PTX, `tileiras` of `ptxas`), then say where cuTile differs, chiefly that the thread level is hidden and the compiler maps a program onto one or more thread blocks, clusters, or warps. The learning notes in `notes/` follow the same rule with a C / CUDA C cousin on every term. The user is new to SSH and to Rust's compiler internals, so those still need to be explained rather than assumed. See "Fork workflow" below before committing, pushing, or relying on CI.
